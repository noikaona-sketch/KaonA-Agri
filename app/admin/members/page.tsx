...
const getRequestedByLabel = (requestedBy: MemberApprovalItem['requestedBy']) =>
  requestedBy === 'leader' ? 'Leader submitted' : 'Self submitted';
        ...
        {pendingMemberApprovals.length === 0 ? (
          ...
          pendingMemberApprovals.map((approval) => (
            <InfoCard
              ...
              subtitle={`${approval.village} • ${approval.citizenIdMasked} • ${approval.requestedAt} • ${getRequestedByLabel(approval.requestedBy)}`}
              meta={<StatusChip status="submitted" />}
              action={
                <div style={{ display: 'grid', gap: 8 }}>
                  {/* Mock actions only for Issue #29 UI preview. */}
