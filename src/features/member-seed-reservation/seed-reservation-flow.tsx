...
export function SeedReservationFlow() {
  ...
  async function load() {
    ...
    setVarieties((data ?? []).map((l: Record<string, unknown>) => ({
      ...
      crop_type:        ((l.seed_varieties as { crop_type: string }[] | null)?.[0]?.crop_type ?? ''),
      days_to_harvest:  ((l.seed_varieties as { days_to_harvest: number }[] | null)?.[0]?.days_to_harvest ?? null),
      notes:            ((l.seed_varieties as { notes: string }[] | null)?.[0]?.notes ?? null),
      planting_guide:   ((l.seed_varieties as { planting_guide: string }[] | null)?.[0]?.planting_guide ?? null),
