...
type CartItem = {
  ...
  unit_price: number;
  qty: number;
  isReservedSeed?: boolean;   // locked — from reservation, cannot change product
};
...
export function AdminPos() {
  ...
  async function loadReservationToCart() {
    ...
    setCart((prev) => {
      ...
      return [...prev, {
        key, type: 'product', product_id: item.product_id,
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 10, overflow: 'hidden' }}>
        ...
        <div style={{ flex: 1, overflowY: 'auto', display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(160px,1fr))', gap: 10, alignContent: 'start' }}>
          ...
          {filtered.map((s) => {
            ...
            // disable seed products when a different reserved seed is already in cart
            const hasReservedSeed  = cart.some((c) => c.isReservedSeed);
            const isSeedProduct    = s.category === 'seed' || s.product_type === 'seed';
            const isLockedOut      = hasReservedSeed && isSeedProduct && !inCart?.isReservedSeed;
            const isDisabled       = s.qty_available <= 0 || isLockedOut;
            return (
              <button key={s.id} onClick={() => !isDisabled && addItem(s)} disabled={isDisabled}
                title={isLockedOut ? 'มีเมล็ดพันธุ์จากการจองอยู่แล้ว — เพิ่มได้เฉพาะปุ๋ย/ยา/อุปกรณ์' : undefined}
            ...
            <div key={item.key} style={{ background: item.isReservedSeed ? '#e8f5e9' : '#fff', borderRadius: 10, padding: '8px 10px', display: 'flex', alignItems: 'center', gap: 8, border: item.isReservedSeed ? '1px solid #a5d6a7' : 'none' }}>
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
                  ...
                  {item.isReservedSeed && <span style={{ fontSize: 10, background: '#1b5e20', color: '#fff', borderRadius: 4, padding: '1px 5px', fontWeight: 700 }}>จอง</span>}
                </div>
                <p style={{ margin: 0, fontSize: 12, color: '#6b7280' }}>{item.unit_price.toLocaleString()} ×</p>
              </div>
              {item.isReservedSeed ? (
                <span style={{ fontWeight: 800, minWidth: 24, textAlign: 'center', fontSize: 15 }}>{item.qty}</span>
              ) : (
                <div style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
