...
const TYPE_BY_CAT: Record<ProductCategory, ProductType> = {
  seed: 'seed', fertilizer: 'fertilizer', pesticide: 'chemical', equipment: 'other', other: 'other',
};
const UNITS = [['kg','กิโลกรัม'],['g','กรัม'],['bag','ถุง/ซอง'],['bottle','ขวด'],['box','กล่อง'],['liter','ลิตร'],['piece','ชิ้น']] as const;
...
type Draft = {
  ...
  crop_type: string; seed_variety: string; days_to_harvest: string; bag_weight_kg: string;
};

const EMPTY: Draft = {
  name: '', product_code: '', category: 'seed', product_type: 'seed',
  unit: 'bag', price_per_unit: '', is_active: true,
  seed_variety_id: '', crop_type: '', seed_variety: '', days_to_harvest: '', bag_weight_kg: '',
};
      ...
      <div className="admin-modal">
        ...
        <div className="admin-modal__body">
          ...
          <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:12 }}>
            ...
            {isSeed && (
              <>
                <SeedVarietySelect value={draft.seed_variety_id} onChange={onVarietyPicked} required={needId} />
                {!needId && <p style={{ gridColumn:'1/-1', margin:0, fontSize:12, color:'#9ca3af' }}>ℹ️ สินค้าปิดใช้งาน — ไม่บังคับเลือกพันธุ์ (บันทึกเป็น draft ได้)</p>}
                ...
                <label className="reg-label">อายุเก็บเกี่ยว (วัน)
                  <input className="reg-input" type="number" min="0" value={draft.days_to_harvest} onChange={(e) => set('days_to_harvest', e.target.value)} />
                </label>
                <label className="reg-label">น้ำหนักต่อถุง (กก.)
                  <input className="reg-input" type="number" min="0" step="0.01" value={draft.bag_weight_kg} onChange={(e) => set('bag_weight_kg', e.target.value)} />
                </label>
              </>
            )}
          </div>
          <label style={{ display:'flex', gap:8, alignItems:'center', fontSize:14, cursor:'pointer', marginTop:10 }}>
            ...
            เปิดใช้งาน
            {needId && !draft.seed_variety_id && <span style={{ fontSize:11, color:'#e65100', marginLeft:4 }}>⚠️ ต้องเลือกพันธุ์เมล็ดก่อนเปิดใช้งาน</span>}
          </label>
        </div>
