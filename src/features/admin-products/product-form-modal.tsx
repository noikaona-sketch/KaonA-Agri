...
export function ProductFormModal({ product, onClose, onSaved }: Props) {
  ...
  const needVarId = isSeed && draft.is_active;
  ...
  function onVarietySelected(id: string, v: SeedVariety | null) {
    setDraft((p) => ({
      ...p,
      seed_variety_id: id,
  ...
  async function save() {
    ...
    if (needVarId && !draft.seed_variety_id) {
      setError('กรุณาเลือกพันธุ์เมล็ด'); return;
    }
  ...
  const set = (k: keyof ProductDraft, v: string | boolean) =>
    setDraft((p) => ({ ...p, [k]: v }));
      ...
      <div className="admin-modal">
        ...
        <div className="admin-modal__body">
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
            ...
            <label className="reg-label">รหัสสินค้า (product_code)
            ...
            <label className="reg-label">หน่วย
              <select className="reg-input" value={draft.unit} onChange={(e) => set('unit', e.target.value)}>
                {[['kg','กิโลกรัม'],['g','กรัม'],['bag','ถุง/ซอง'],['bottle','ขวด'],['box','กล่อง'],['liter','ลิตร'],['piece','ชิ้น']].map(([v,l]) => <option key={v} value={v}>{l}</option>)}
              </select>
            </label>
            ...
            {isSeed && (
              <>
                <SeedVarietySelect
                  value={draft.seed_variety_id}
                  onChange={onVarietySelected}
                  required={needVarId}
                />
                {!needVarId && (
                  <p style={{ gridColumn: '1/-1', margin: 0, fontSize: 12, color: '#9ca3af' }}>
                    ℹ️ สินค้าที่ปิดใช้งาน — ไม่บังคับเลือกพันธุ์เมล็ด
                  </p>
                )}
