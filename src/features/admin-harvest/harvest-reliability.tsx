...
export type ReliabilityStats = {
  ...
  pending:    number;
  total:      number;
  noShowRate: number;  // primary when no_show > 0
  cancelRate: number;  // backwards compat
};
...
export function getReliabilityLevel(stats: ReliabilityStats): ReliabilityLevel {
  ...
  const rate = stats.no_show > 0 ? stats.noShowRate : stats.cancelRate;
  if (rate <= 10) return 'ดี';
  if (rate <= 30) return 'เฝ้าระวัง';
...
export function ReliabilityBadge({ stats }: { stats: ReliabilityStats }) {
  ...
  return (
    <span style={{
      ...
      {st.icon} {level} ({stats.no_show > 0 ? `${stats.noShowRate}% ไม่มา` : `${stats.cancelRate}% ยกเลิก`})
    </span>
  );
}
...
export function useMemberReliability(memberId: string | null): {
  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      const no_show   = data.filter((r) => r.status === 'no_show').length;
      ...
      setStats({ completed, cancelled, no_show, pending, total, cancelRate, noShowRate });
      setLoading(false);
    })();
  }, [memberId]);
